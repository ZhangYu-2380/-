...
const alerts: Alert[] = [
  { id: "ALT-240916-001", level: "紧急", ip: "10.21.8.17", device: "生产数据库-01", system: "资金清算", source: "Zabbix", time: "13:28:41", content: "主库连接数达到 96%，存在业务阻塞风险", status: "未确认" },
  { id: "ALT-240916-002", level: "严重", ip: "10.21.5.32", device: "应用节点-A03", system: "交易中台", source: "应用日志", time: "13:25:18", content: "订单路由服务连续 5 分钟错误率超过阈值", status: "处理中" },
  { id: "ALT-240916-003", level: "严重", ip: "10.21.9.46", device: "文件服务器-02", system: "批处理平台", source: "Prometheus", time: "13:22:06", content: "磁盘可用空间低于 10%", status: "未确认" },
  { id: "ALT-240916-004", level: "警告", ip: "10.21.6.11", device: "消息队列-01", system: "消息总线", source: "Kafka", time: "13:18:52", content: "消费延迟超过 120 秒", status: "处理中" },
  { id: "ALT-240916-005", level: "警告", ip: "10.21.3.84", device: "网关节点-G02", system: "统一接入", source: "网络监控", time: "13:16:39", content: "上游链路丢包率 2.4%", status: "未确认" },
  { id: "ALT-240916-006", level: "提示", ip: "10.21.7.19", device: "报表服务-01", system: "运营报表", source: "系统日志", time: "13:12:15", content: "定时报表任务执行耗时较日均值上升 30%", status: "已恢复" },
  { id: "ALT-240916-007", level: "警告", ip: "10.21.5.28", device: "应用节点-A01", system: "交易中台", source: "APM", time: "13:07:44", content: "接口 P95 响应时间超过 800ms", status: "已恢复" },
  { id: "ALT-240916-008", level: "提示", ip: "10.21.10.8", device: "备份服务器-01", system: "运维平台", source: "备份任务", time: "12:56:20", content: "增量备份任务已延后启动", status: "已恢复" },
];
      ...
      <section className="alert-workspace">
            ...
            <table>
              ...
              <tbody>
                {visibleAlerts.map((alert) => <tr key={alert.id} className={selected?.id === alert.id ? "selected" : ""} onClick={() => setSelectedId(alert.id)}>
                  ...
                  <td>{alert.system}</td><td>{alert.source}</td><td className="time-cell">今天 {alert.time}</td><td className="content-cell">{alert.content}</td><td><span className={`status-label ${alert.status === "未确认" ? "open" : alert.status === "处理中" ? "handling" : "resolved"}`}>{alert.status}</span></td>
                </tr>)}
              </tbody>
            </table>
          </div>
        ...
        {selected && <aside className="alert-detail" aria-label="告警详情">
          ...
          <dl>
            ...
            <div><dt>发生时间</dt><dd>2026-09-16 {selected.time}</dd></div>
          </dl>
