...
此仓库已配置为纯静态演示站：页面使用内置模拟数据，不包含登录、接口或数据库能力。推送到 `main` 分支后，GitHub Actions 会构建并发布到 GitHub Pages。

首次发布时，在仓库的 **Settings → Pages → Build and deployment** 中选择 **GitHub Actions**。若仓库名为 `broker-flow-monitor`，访问地址通常为 `https://<GitHub用户名>.github.io/broker-flow-monitor/`。

GitHub Pages 不适合保存真实券商运行数据、账号或密钥；接入真实数据时应改用带后端和访问控制的部署方式。
